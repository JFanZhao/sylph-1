...
public class DateSub extends ScalarFunction {
    private static final Logger logger = LoggerFactory.getLogger(DateSub.class);

    private static final String DEFAULT_DATE_TIME_FORMAT = "yyyy-MM-dd HH:mm:ss";
    private static final String DEFAULT_DATE_FORMAT = "yyyy-MM-dd";

    @Description("VARCHAR DATE_SUB(TIMESTAMP time, INT days)")
    public String eval(Timestamp timestamp, Integer days) {
        ...
        String result = null;
        try {
            result = new DateTime(timestamp).minusDays(days).toString(DEFAULT_DATE_FORMAT);
        } catch (Exception e) {
            logger.warn("DATE_SUB 调用异常", e);
        }
        return result;
    }

    @Description("VARCHAR DATE_SUB(TIMESTAMP time,VARCHAR to_format, INT days)")
    public String eval(Timestamp timestamp, String toFormat, Integer days) {
        ...
        String result = null;
        try {
            result = new DateTime(timestamp).minusDays(days).toString(toFormat);
        } catch (Exception e) {
            logger.warn("DATE_SUB 调用异常", e);
        }
        return result;
    ...
    @Description("VARCHAR DATE_SUB(VARCHAR time, INT days)")
    ...
    @Description("VARCHAR DATE_SUB(VARCHAR time,VARCHAR to_format, INT days)")
    ...
    @Description("VARCHAR DATE_SUB(VARCHAR time,VARCHAR from_format, VARCHAR to_format, INT days)")
    public String eval(String date, String fromFormat, String toFormat, Integer days) {
        ...
        String result = null;
        try {
            result = DateTimeFormat.forPattern(fromFormat).parseDateTime(date).minusDays(days).toString(toFormat);
        } catch (Exception e) {
            logger.warn("DATE_SUB 调用异常", e);
        }
        return result;
